...
Bytes NoiseState::serialize() const {
    ...
    w.bytes(m_rs);
    w.bytes(m_e);
    w.bytes(Bytes{});                // m_ek — intentionally empty
    w.bytes(m_re);
...
NoiseState NoiseState::deserialize(const Bytes& data) {
    ...
    if (!r.ok() || version < 1 || version > 5) return ns;
    ...
    ns.m_e  = r.bytes();
    {
        // Read and immediately discard m_ek from the wire.  v5 encoders
        // write empty; v4 encoders wrote the real key.  Either way the
        // deserialized state holds nothing — readMessage2 will fail
        // closed for any initiator that reloads a pending handshake
        // across a crash, and SessionManager purges the stale row.
        Bytes legacyEk = r.bytes();
        if (!legacyEk.empty()) sodium_memzero(legacyEk.data(), legacyEk.size());
    }
    ns.m_re = r.bytes();
