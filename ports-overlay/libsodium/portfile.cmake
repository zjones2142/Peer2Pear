...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
    ...
    # Peer2Pear overlay: upstream's autotools cross-compile detection passes
    # --host=aarch64-apple-darwin for iOS targets.  Autoconf sees that as
    # build == host, decides it's native, and tries to RUN the conftest
    # binary — which fails because an iOS Simulator / iPhoneOS binary can't
    # execute on macOS.  Force a proper iOS host triple so autoconf marks
    # the build as cross-compiling and skips the run check.
    #
    # vcpkg-make reads VCPKG_MAKE_BUILD_TRIPLET to override the auto-detected
    # triplet (see vcpkg/ports/vcpkg-make/vcpkg_make.cmake:225).
    if(VCPKG_TARGET_IS_IOS)
        # Both device and simulator use `aarch64-apple-ios`.  Autoconf's
        # config.sub doesn't recognize `-ios-simulator` as a kernel+OS pair;
        # the iphonesimulator SDK already applies the simulator ABI via the
        # -isysroot flag the toolchain passes through CC.
